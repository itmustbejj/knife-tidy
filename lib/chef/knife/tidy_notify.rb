...
class Chef
  class Knife
    class TidyNotify < Knife
      ...
      def generate_email(report_data, organization, recipients)
...
#{generate_total_cookbook_table(report_data, organization)}
<br/>
#{generate_unused_cookbook_table(report_data, organization)}
<br/>
#{generate_node_table(report_data, organization)}
--#{mime_boundary}
<% report_files.each do |r_file| %>
<% unless report_data[organization].empty? %>
Content-Transfer-Encoding:base64
Content-Type: application/json; name = "#{r_file}"
Content-Disposition: attachment; filename = "#{r_file}"
--#{mime_boundary}
<% end %>
<% end %>
MESSAGE_END
      ...
      def generate_total_cookbook_table(report_data, organization)
        ...
        table_body = report_data[organization]['_cookbook_count.json'].map { |cookbook_name, cookbook_count| "<tr><td>#{cookbook_name}</td><td>#{cookbook_count}</td></tr>" }.join("\n")
        table_start + header_string + table_body + table_end
      end

      def generate_total_unused_table(report_data, organization)
        ...
        table_body = report_data[organization]['_unused_cookbooks.json'].map { |cookbook_name, cookbook_versions| "<tr><td>#{cookbook_name}</td><td>#{cookbook_versions.join('<br>')}</td></tr>" }.join("\n")
        table_start + header_string + table_body + table_end
      end
      ...
      def generate_node_table(report_data, organization)
        ...
        table_body = report_data[organization]['_stale_nodes.json']['list'].map { |node_name| "<tr><td>#{node_name}</td></tr>" }.join("\n")
        table_start + header_string + table_body + table_end
      end
