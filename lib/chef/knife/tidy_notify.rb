    ...
    class TidyNotify < Knife
      ...
      def run
        ...
        org_names.each do |org|
          ...
          email_content = generate_email(reports, org, email_recipients, reports_dir, report_file_suffixes)
      ...
      def generate_email(report_data, organization, recipients, reports_dir, report_file_suffixes)
...
MESSAGE_END
        ...
        report_file_suffixes.each do |suffix|
          unless report_data[organization][suffix].empty?
            message += <<MESSAGE_END
--#{mime_boundary}
